#!/usr/bin/env python3
"""
SAIBATIN AZURA 1.0 - ALL-IN-ONE MISSION SYSTEM (improved)
Perbaikan:
 - dynamic frame center (tidak pakai konstanta 320)
 - threading.Event untuk shutdown terkoordinasi
 - Lock untuk shared gps data
 - rate limiting pengiriman frame (SEND_FPS)
 - bounded geolog & pathPoints
 - safer socket.io reconnect/backoff and exception handling
 - resource cleanup (camera.release, sio.disconnect)
"""
...
# Dual camera setup
CAMERA_NAVIGATION = 0  # Logitech C270 untuk navigasi + surface
CAMERA_UNDERWATER = 1  # Brica untuk underwater
...
# ========== GLOBAL ==========
sio = socketio.Client()
camera_nav = None  # Logitech untuk navigasi
camera_underwater = None  # Brica untuk underwater
...
def send_dashboard_update():
    global data_send_count, gps_data_event
    while True:
        try:
            gps_data_event.wait()
...
def init_dual_cameras():
    """Initialize both cameras: navigation (Logitech) and underwater (Brica)"""
    ...
    try:
        ...
        if cam_nav.isOpened():
            ...
            if ret:
                camera_nav = cam_nav
                print(f"✅ Navigation Camera (Logitech) initialized on /dev/video{CAMERA_NAVIGATION}")
            else:
                cam_nav.release()
                print(f"❌ Navigation camera read failed")
        else:
            print(f"❌ Cannot open navigation camera")
    except Exception as e:
        print(f"❌ Navigation camera error: {e}")
    
    # Initialize underwater camera (Brica) - optional
    try:
        ...
        if cam_under.isOpened():
            ...
            ret, _ = cam_under.read()
            if ret:
                camera_underwater = cam_under
                print(f"✅ Underwater Camera (Brica) initialized on /dev/video{CAMERA_UNDERWATER}")
            else:
                cam_under.release()
                print(f"⚠️ Underwater camera read failed (optional)")
        else:
            print(f"⚠️ Cannot open underwater camera (optional)")
    except Exception as e:
        print(f"⚠️ Underwater camera error (optional): {e}")
    
    # Return True jika minimal navigation camera OK
...
def capture_surface_photo(frame_bgr):
    """Capture surface photo using navigation camera (Logitech)"""
    try:
        _, buffer = cv2.imencode('.jpg', frame_bgr, [cv2.IMWRITE_JPEG_QUALITY, 95])
        jpg_as_text = base64.b64encode(buffer).decode('utf-8')
        
        with gps_lock:
            loc = f"{latest_gps_data.get('Latitude_DD', 'N/A')}, {latest_gps_data.get('Longitude_DD', 'N/A')}"
        ...
        print("📸 Surface photo captured (Logitech)!")
        return True
    except Exception as e:
        print(f"⚠️ capture_surface_photo error: {e}")
        return False

def capture_underwater_photo():
    """Capture underwater photo using Brica camera"""
    global camera_underwater
    
    if not camera_underwater or not camera_underwater.isOpened():
        print("⚠️ Underwater camera not available")
        return False
    
    try:
        ret, frame_bgr = camera_underwater.read()
        if not ret:
            print("⚠️ Failed to read from underwater camera")
            return False
        ...
        if sio.connected:
            sio.emit('underwater_photo', photo_data)
        print("📸 Underwater photo captured (Brica)!")
        return True
    except Exception as e:
        print(f"⚠️ capture_underwater_photo error: {e}")
        return False

# ========== CAMERA LOOP ==========
def camera_stream_loop():
    """Main camera loop: detect balls, draw overlay, send frames"""
    global mission_status, camera_nav, balls_passed, pathPoints, geolog
    last_send = 0.0
    frame_count = 0
    send_interval = 1.0 / SEND_FPS if SEND_FPS > 0 else 0.1
    
    # Trigger flags untuk capture (GPS-based atau manual)
    surface_captured = False
    underwater_captured = False

    try:
        while not stop_event.is_set():
            if not camera_nav or not camera_nav.isOpened():
                time.sleep(0.5)
                continue

            ret, frame_bgr = camera_nav.read()
            if not ret or frame_bgr is None:
                time.sleep(0.05)
                continue

            frame_height, frame_width = frame_bgr.shape[:2]
            center_frame = int(frame_width / 2)
            ...
            try:
                ...
                # Send MJPEG frame (binary) untuk video stream
                if sio.connected:
                    try:
                        sio.emit('mjpeg_frame', {'jpeg': jpeg_bytes})
                    except Exception as e:
                        print(f"⚠️ emit mjpeg_frame failed: {e}")
                
                # Send metadata (ball detection info) - rate limited
                nowt = time.time()
                if SEND_FPS <= 0 or (nowt - last_send) >= send_interval:
                    last_send = nowt
                    if sio.connected:
                        camera_data = {
                            'red_ball': red_ball,
                        ...
                        try:
                            ...
                            print(f"⚠️ emit camera_frame failed: {e}")
                    
                    frame_count += 1
                    if frame_count % (SEND_FPS * 10 if SEND_FPS>0 else 100) == 0:
                        print(f"📷 {frame_count} frames | {mission_status}")
                        
            except Exception as e:
                print(f"⚠️ frame encoding/sending error: {e}")
...
# ========== COMMAND INTERFACE (MANUAL CAPTURE) ==========
def command_interface():
    """Interactive command interface"""
    global AUTONOMOUS_MODE
    
    print("\n🎮 Command Interface Active")
    print("   Commands: arm, disarm, guided, hold, auto, manual")
    print("   Photo: surface, underwater")
    ...
    while True:
        try:
            ...
            if cmd == "surface":
                # Manual trigger surface photo
                if camera_nav:
                    ret, frame = camera_nav.read()
                    if ret:
                        capture_surface_photo(frame)
                    else:
                        print("❌ Failed to capture frame")
                else:
                    print("❌ Navigation camera not available")
                    
            elif cmd == "underwater":
                # Manual trigger underwater photo
...
def main():
    ...
    camera_ok = init_dual_cameras()
    
    if not camera_ok:
        print("❌ Navigation camera failed to initialize. Exiting.")
        stop_event.set()
        return
